        ...
        <div class="container add-user-form">
          ...
          <div class="row mt-3">
            ...
            <div class="mb-4 col-sm-12 col-md-3">
              <h6>Aadhar Card :</h6>
              <input class="demo1" type="file" wire:model="document" value="drage and drop file here or select files" />
              @error('document') <span class="text-danger">{{ $message }}</span>@enderror
              @if(!is_object($document)&& $document!='')
              <button wire:click.prevent="download('{{ $document }}')" type="button" class="btn btn-success mb-0 text-white mt-3">Download</button>
              @endif
            </div>
          </div>
